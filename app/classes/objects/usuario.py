...
class usuario(object):
    ...
    def nombres(self,value):
        if value != None and (len(value) < 3 or len(value) > 32) :
    ...
    @tipo.setter
    def tipo(self,value):
        if value != None and (value != "Administrador" and value != "Usuario"):
    ...
    def usuario(self,value):
        if value != None and (len(value) < 3 or len(value) > 24):
    ...
    def email(self,value):
        import re
        if value != None and not re.search("^((\w|ñ){1,}(\.){0,1}){1,}@(\w|ñ){3,}((\.)[ñA-Za-z]{2,3}){1,}",value) and self.usuario != "administrador.scada":
